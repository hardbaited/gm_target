...
local exports = {
  ...
  AddBoxZone = function(name,center,length,width,options,targetOptions)
    ...
    return mTarget.addInternalBoxZone(
      name,
      targetOptions.title or targetOptions.options[1].label or name:upper(),
      targetOptions.options[1].icon,
      center,
      length,
      width,
      options,
      targetOptions.distance or false,
      targetOptions.options[1].action and NewFunction or false,
      items,
      {},
      GetInvokingResource(),
      targetOptions.canShow,
      targetOptions.options[1].job,  -- Correctly pass job from targetOptions
      targetOptions.options[1].item  -- Pass item from targetOptions
    )
  end,
